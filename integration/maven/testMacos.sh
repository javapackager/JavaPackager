...
test_app () {
  open /Applications/JavaPackager\ Demo.app
...
hdiutil detach /Volumes/JavaPackagerDemo || true
sudo rm -rf /Applications/JavaPackager\ Demo.app /tmp/javapackager-testfile.txt

pushd app
../mvnw compile javapackager:package@packageForMac
popd
...
rm -rf app/target/JavaPackager\ Demo


hdiutil attach app/target/JavaPackager\ Demo_1.0.0-SNAPSHOT.dmg
cp -r /Volumes/JavaPackagerDemo/JavaPackager\ Demo.app /Applications
hdiutil detach /Volumes/JavaPackagerDemo

test_app

rm -rf /Applications/JavaPackager\ Demo.app /tmp/javapackager-testfile.txt

# This sudo is a bit annoying for local testing, but find in the actions build
sudo installer -pkg app/target/JavaPackager\ Demo_1.0.0-SNAPSHOT.pkg -target /
